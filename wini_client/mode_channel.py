...
# The UI reader queues each inbound line into a fixed 512-byte slot (ipc.c
# IPC_LINE_MAX) and drains it into a 512-byte buffer, so a longer line is
# truncated mid-JSON and fails to parse. Keep every command line well under that;
# the ModeChannelSink also caps individual fields, this is the last-ditch guard.
MAX_LINE = 480
