...
class Run:
    ...
    def __init__(self, letter: str, path: Path | None = None) -> None:
        self.path = path
        self.letter = letter
    ...
    def save(self, completed: bool) -> None:
        ...
        with closing(_conn(self.path)) as c:
            c.execute(
                "INSERT INTO lesson_run (letter, started_at, finished_at, completed,"
                " touch_attempts, touch_correct, speech_attempts, speech_matched, seconds)"
                " VALUES (?,?,?,?,?,?,?,?,?)",
                (self.letter, self.started.isoformat(timespec="seconds"),
...
def completed_letters(path: Path | None = None) -> list[str]:
    with closing(_conn(path)) as c:
        rows = c.execute("SELECT DISTINCT letter FROM lesson_run WHERE completed=1"
                         " ORDER BY letter").fetchall()
    return [r["letter"] for r in rows]


def next_letter(order: list[str], path: Path | None = None) -> str:
    ...
    """
    done = set(completed_letters(path))
    for ch in order:
        if ch not in done:
            return ch
    return order[0]
...
def parent_summary(path: Path | None = None) -> dict:
    ...
    with closing(_conn(path)) as c:
        ...
        recent = c.execute(
            "SELECT letter, date(started_at) day, touch_attempts, touch_correct,"
            " speech_attempts, speech_matched FROM lesson_run"
            " ORDER BY id DESC LIMIT 20").fetchall()

    done = completed_letters(path)
    return {
