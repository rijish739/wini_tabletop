...
if __name__ == "__main__":
    ...
    # Ensure dependencies and contracts imports resolve when run as a script
    pkg_dir = Path(__file__).resolve().parent.parent
    if str(pkg_dir) not in sys.path:
        sys.path.insert(0, str(pkg_dir))

    from runtime.contracts import DeviceCapabilities, TurnBudgets
    ...
    def _classify_route(text: str):
        ...
        social_keywords = ["hello", "hi", "hii", "hiii", "hey", "who are you", "what is your name", "what can you do", "help", "bye", "goodbye", "good morning", "good evening"]
    ...
    print("=" * 75)
    print("   DIRECT PHASE 1 MODULE TESTER (control.py)")
    print("=" * 75)
    print("File Running     : cloud_run_service/interaction_control/control.py")
    print("Execution Target : Direct InteractionControl.control() method invocation")
    print("\nType your input below and press Enter (or 'exit' / 'quit' to stop).")
    print("-" * 75)

    while True:
        try:
            user_input = input(f"\n[control.py Turn {turn_counter} Input] > ").strip()
        ...
        if user_input.lower() in ("exit", "quit", "q"):
            print("Exiting control.py direct tester.")
            break
        ...
        print("\n" + "=" * 70)
        print(f"    DIRECT OUTPUT FROM InteractionControl.control() (Turn {turn_counter})")
        print("=" * 70)
        print(f"📍 Class & Method  : InteractionControl.control()")
        ...
        if decision.disposition == InteractionDisposition.CONTINUE_LEARNING:
            ...
            print(f"   Continuity Payload : {decision.continuity}")
            ...
            print("\n⚙️  Outcome         : ADMITTED_TO_PHASE_2 (Phase 1 Complete)")

        else:
            print("\n⏹️  [FAST-PATH EXIT IN PHASE 1]")
            ...
            if outcome.state_changes:
                ...
                for sc in outcome.state_changes:
                    print(f"     - Path {sc.path}: {sc.value}")
            print("\n⚙️  Outcome         : COMPLETED_IN_PHASE_1 (No downstream calls)")
