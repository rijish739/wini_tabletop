...
def test_scene_author_renders_without_skips():
    ...
    # `figures/` is a DEVICE-side package: the lean Cloud Run image ships no server-side
    # renderer (see tutor_loop 5d), so this test skips there instead of failing.
    try:
        from figures.scene_render import render_beat_frame
    except ImportError as e:
        raise SkipTest(f"figures/ not present in this image: {e}") from e
    from .scene_author import layout_scene
