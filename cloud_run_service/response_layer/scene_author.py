...
def fast_extract_board_lines(answer: str, concept_id: str | None = None,
                             evidence: list[dict] | None = None
                             ) -> tuple[str, list[str], dict | None]:
    """Fast, deterministic (0 ms, 0 LLM calls) board line & parabola extractor.
    Extracts math equations, formulas, definitions, and quadratic coefficients directly
    from answer text (or RAG evidence if answer sentence 0 is an intro line).
    """
    ...
    def scan_text(txt: str):
        ...
        for raw in raw_lines:
            s = raw.strip()
            if "=" in s or "±" in s or "√" in s or "x²" in s or "x^2" in s or "b²" in s or "b^2" in s:
                eq_m = re.search(r"([A-Za-z0-9_\^+\-*/().±√≤≥≠\s=]{3,40}(?:=[A-Za-z0-9_\^+\-*/().±√≤≥≠\s]{1,40})+)", s)
                chunk = eq_m.group(1).strip() if eq_m else s
                chunk = re.sub(r"[.,;!?]+$", "", chunk).strip()
                if 3 <= len(chunk) <= MAX_LINE_CHARS and chunk not in lines:
                    lines.append(chunk)

    # 1. Scan primary answer text
    scan_text(answer)

    # 2. If sentence 0 had no equations, fall back to RAG evidence items
    if len(lines) < MIN_LINES and evidence:
        for item in evidence:
            txt = (item.get("text") or item.get("prompt") or item.get("note") or "")
            scan_text(txt)
            if len(lines) >= MAX_LINES:
                break

    # Extract parabola coefficients if text or evidence mentions a specific quadratic
    parabola = None
    combined_text = f"{answer} " + " ".join((e.get("text") or "") for e in (evidence or []))
    quad_m = re.search(r"(-?\d*(?:\.\d+)?)\s*\*?\s*x\s*(?:\^2|²)\s*([+-]\s*\d*(?:\.\d+)?)\s*\*?\s*x\s*([+-]\s*\d*(?:\.\d+)?)", combined_text)
    if quad_m:
        try:
            sa, sb, sc = quad_m.groups()
            sa = sa.replace(" ", "")
            a = float(sa) if sa not in ("", "+") else (1.0 if sa != "-" else -1.0)
            sb = sb.replace(" ", "")
            b = float(sb) if sb not in ("", "+") else (1.0 if sb != "-" else -1.0)
            sc = sc.replace(" ", "")
            c = float(sc)
            if a != 0:
    ...
    title = (concept_id or "Mathematics").replace("_", " ").replace("concept::", "").title()[:30]
    return title, lines[:MAX_LINES], parabola
...
def author_scene_from_answer(answer: str, concept_id: str | None = None,
    ...
    lay them out. Guaranteed to return a valid scene spec in 0 ms.
    ...
    # 1. Fast deterministic extraction (0 ms, 0 LLM calls)
    title, lines, parabola = fast_extract_board_lines(answer, concept_id, evidence)
    grounded = _ground_lines(lines, answer) if lines else []

    if parabola or len(grounded) >= MIN_LINES or len(lines) >= MIN_LINES:
        use_lines = grounded if len(grounded) >= MIN_LINES else lines
        if parabola:
            try:
                return parabola_scene(concept_id, parabola["a"], parabola["b"], parabola["c"], title)
            except Exception:  # noqa: BLE001
                pass
        if len(use_lines) >= MIN_LINES:
            return layout_scene(concept_id, title, use_lines)

    # 2. Instant clean text card fallback (0 ms) — guarantees early turn_meta fires on sentence 0
    clean_title = (concept_id or "Mathematics").replace("_", " ").replace("concept::", "").title()[:30]
    clean_text = re.sub(r"\s+", " ", answer.strip())[:42]
    if clean_text:
        return layout_scene(concept_id, clean_title, [clean_text])
