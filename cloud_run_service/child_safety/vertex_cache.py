"""Vertex context cache for the child-safety static block (§7.1).

Its own cache, its own record file, its own lifecycle — deliberately not shared
with ``perception/vertex_cache.py``. A safety prompt fused into perception's would
be re-validated every time concept resolution changes and could not be evaluated
independently, which §14's human-reviewed / versioned requirement forbids.

Graceful by construction: a missing, expired or stale cache NEVER breaks a turn.
``active_name()`` returns a resource name only if the record exists, has not
expired (2-minute margin), the prompt hash still matches, and the model id still
matches. Otherwise the call sends the full system instruction and behaves
identically, only costing more.

CLI:
    python -m child_safety.vertex_cache --create [--ttl-hours 24]
    python -m child_safety.vertex_cache --status
    python -m child_safety.vertex_cache --delete
"""
...
def create(ttl_hours: float = 24.0) -> dict:
    """Create (replace) the cache from the current prompt-of-record. Billed:
    one-time cached-token ingestion plus small hourly storage while the TTL runs."""
    from google.genai import types
    import llm_vertex

    client = llm_vertex._client(config.VERTEX_SAFETY_LOCATION)
    old = _load_record()

    cache = client.caches.create(
        model=config.resolved_model(),
        config=types.CreateCachedContentConfig(
            display_name="wini-child-safety-static",
    ...
    record = {
        ...
        "region": config.VERTEX_SAFETY_LOCATION,
...
def delete() -> bool:
    ...
    try:
        ...
        llm_vertex._client(config.VERTEX_SAFETY_LOCATION).caches.delete(
            name=record["name"]
        )
    except Exception:  # noqa: BLE001
        pass
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Vertex context cache for the child-safety static block"
    )
