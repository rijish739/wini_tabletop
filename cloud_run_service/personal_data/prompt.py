"""The personal-data prompt-of-record and its version stamps.

Written **against `docs/architecture/PERSONAL_DATA_CONTRACT.md` §3, §4, §5 and §14
only** — the class definitions in that document are the entire source. It was NOT
written by reading `eval/corpora/pii/*.jsonl`. The blindness rule runs in both
directions: a corpus written from the prompt measures the prompt, and a prompt tuned
on the corpus measures the corpus. That symmetry is why the contract document exists
separately from both.

Two things this prompt may never do, enforced again in the schema and a third time in
the validation belt (`detector._validate`):

* it never returns **character spans** — §4 rejects them, because LLMs are unreliable
  at exact offset arithmetic and a wrong offset redacts the wrong half of a sentence;
* it never returns a **rewritten utterance** — that hands the model a licence to alter
  the arithmetic, which is exactly the utility failure MathEd-PII measures.

It returns the **verbatim substring** and its class, and nothing else.

Changing anything in this file changes ``PROMPT_VERSION``, which invalidates the eval
cache and the Vertex context cache and obliges a re-run of both §12 corpora before
deploy.
"""
...
PROMPT_VERSION = "personal-data-prompt-v1"

#: Bump on ANY change to the response schema (personal_data/schema.py).
SCHEMA_VERSION = "personal-data-schema-v1"

#: The nine enum members (§3). ``OTHER_IDENTIFIER`` is the honest residual and is
#: ALWAYS available — a verdict of OTHER_IDENTIFIER is a real finding, not a miss.
...
def prompt_hash() -> str:
    ...
    The eval cache is keyed by this: a prompt change invalidates the cache, and
    mixing caches across prompt hashes is a hard failure rather than a warning. It
    covers the version stamps as well as the text, so a schema bump also invalidates.
    """
    payload = "\n".join((PROMPT_VERSION, SCHEMA_VERSION, STATIC_BLOCK))
    return hashlib.sha256(payload.encode("utf-8")).hexdigest()[:16]


def dynamic_prompt(*, text: str, recent_context: tuple = ()) -> str:
    """The per-turn prompt (§14).

    Sees the ONE preceding exchange -- ``session["context"][-2:]``, the learner's last
    turn and Wini's reply to it. That window is the only thing that catches the split
    disclosure: the tutor asks something and the child answers *"it's 98765"*, which
    without context is indistinguishable from an answer and must be left alone.

    Nothing else from the session reaches this prompt. There is no count of prior
    disclosures and no prior class label — §9 forbids the standing behavioural record
    that would be needed to supply one, and telling the model "this child disclosed an
    address before" invites it to confirm rather than detect.
    """
    lines = []
    if recent_context:
        rendered = "; ".join(
        ...
        if rendered:
            lines.append(f"PRECEDING EXCHANGE (evidence only, never a target):\n{rendered}")
    preamble = ("\n\n".join(lines) + "\n\n") if lines else ""
    return (
        "Find the personal identifiers in this one student utterance.\n\n"
        f"{preamble}"
        f"STUDENT UTTERANCE:\n{text}"
    )
