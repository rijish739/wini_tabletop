"""Canonical in-process Turn runtime used by the typed coordinator."""
...
class TurnRuntimeFailure(RuntimeError):
...
class TurnRuntime:
    """Join typed Module outcomes, commit the Turn, and build its result."""

    name = "canonical_turn_runtime"

    def __init__(
        self,
        *,
        turn_behavior: Callable[..., Mapping[str, Any]],
        commit_state: Callable[[], None],
        state: Any,
    ) -> None:
        self._turn_behavior = turn_behavior
    ...
    def retrieval_request(self, turn_input: TurnInput, observation, pedagogical):
        ...
        engine = getattr(self._turn_behavior, "__self__", None)
    ...
    def _execute(self, turn_input: TurnInput, interaction_outcome=None,
        ...
        # Imported lazily to keep the runtime/coordinator modules acyclic.
        from .coordinator import LOGICAL_TURN_PHASES, TurnExecution
        ...
        try:
            ...
            if decision is not None and decision.disposition.value == "complete":
                ...
                compatibility = dict(self._turn_behavior(controlled_text, **kwargs))
                if decision is not None:
            ...
            signal = FailureSignal(
                # Preserve the externally observed terminal capability label;
                # the implementation is now the canonical runtime, not a
                ...
                valid_outcome=False,
                context={"runtime": self.name},
            )
            raise TurnRuntimeFailure(original=exc, signal=signal) from exc
        ...
        commit = TurnCommit(
            commit_id="turn_commit_" + hashlib.sha256(
        ...
        result = TurnResult(
            ...
            realization=realization or RealizationReceipt(
                ...
                details={
                    "source": self.name,
                    "observation": "presentation_receipt_not_supplied",
                },
            ),
            commit=commit,
            failures=(
        ...
        return TurnExecution(
            ...
            measurements={
                "turn_runtime_turns": 1,
                "turn_runtime_unprojected_phases": len(LOGICAL_TURN_PHASES),
                # Frozen metrics compatibility during the contract migration.
