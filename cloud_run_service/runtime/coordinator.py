...
class TurnCoordinator:
    ...
    def __init__(
        ...
        *,
        ...
        observation = None
        if self._utterance_intake is not None and getattr(turn_input, "utterance", None) is not None:
            try:
        ...
        if observation is not None and hasattr(observation, "authorization"):
            try:
        ...
        if self._perception is not None and not _skip_perception:
            perception_request = self._runtime.perception_request(turn_input)
            if observation is not None:
                try:
                    perception_request = replace(perception_request, observation=observation)
                except Exception:
                    pass
