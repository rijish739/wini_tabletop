...
- [x] `lark` added to `requirements.txt`; grammar at `cloud_run_service/utterance_intake/grammar/`;
...
- [x] v1 scope = what `math_grade` grades **plus exponents**; `PASSTHROUGH` is the common legitimate
...
- [x] Assessment grades `MathParse.interpretation` **iff `ACCEPT`**, falls back to `math_grade` on
  `PASSTHROUGH`, never grades on `REFUSE` — does not breach the one-published-string rule.
- [x] The four measured confident false negatives each become a correct parse **or** a refusal — never a
  silent wrong (fixtures assert this).
- [x] Refusal rate measured over **claimed maths spans**, recorded against the 5%–15% band (calibration
...
- [x] Perf microbenchmark guards Earley blowup (p95 > 3× baseline fails, CI-only); baseline + dated
