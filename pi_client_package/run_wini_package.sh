...
# NOTE: env vars go through `setsid env VAR=... cmd` — `setsid VAR=... cmd`
# treats VAR=... as the program to exec ("setsid: failed to execute
# WINI_BOARD_BUDDY=1") so the client never starts and the UI hangs on its splash.
setsid env WINI_BOARD_BUDDY=1 WINI_BB_PATH="$HOME/board_buddy_sandbox" \
    .venv/bin/python -u -m wini_client.client \
